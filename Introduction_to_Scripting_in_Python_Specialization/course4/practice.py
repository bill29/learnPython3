...
print('Qua nay rut ra mot dieu la neu la list tuples se co the vong lap tai do')

list_tuple = [('minh', 'dan'), ('nguyen tri', 'hieu')]
for (ho, ten) in list_tuple:
    print(ho)

with open('minhdan.txt','w', newline='') as file:
    file_write = csv.writer(file, delimiter=',', quoting=csv.QUOTE_MINIMAL)
    file_write.writerow(str(list_tuple[0]))


print(list_tuple[0])

my_tuple = list_tuple[1]
print(my_tuple)
